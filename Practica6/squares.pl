...
% Restriccions: 
%   1. Que el quadrat estigui dins del gran
%   2. Que els quadrats no se solapin

% Variables: Coordenada del vertex superior esquerre del quadrat

% Domini: nombre d'elements del quadrat

insideBigSquare(_,_,[],[]).
insideBigSquare(_,Big,[S|Sides],[V|ListVars]):- V #=< Big+1-S, insideBigSquare(_,Big,Sides,ListVars).
...
% Agafem el quadrat escollit i mirem si es sobreposa amb algun altre
nonoverlapping2(_,[],_,[],_,[]).
nonoverlapping2(X,[X2|Sides],R,[R2|RowVars],C,[C2|ColVars]):-
	nonoverlapping3(X,X2,R,C,R2,C2), 
	nonoverlapping2(X,Sides,R,RowVars,C,ColVars). 

% Mirem si se sobreposa
nonoverlapping3(X,X2,R,C,R2,C2):- 
	X+R #=< R2 #\/      % MIDA+FILA <= FILA2 v MIDA+COLUMNA <= COLUMNA2 V MIDA2+FILA2 <= FILA V MIDA2+COLUMNA2 <=  COLUMNA
	X+C #=< C2 #\/
	X2+R2 #=< R #\/
       	X2+C2 #=< C.