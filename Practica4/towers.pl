...
satVariable( hasTower(V) ):- village(V).
...
writeClauses(MaxNumTowers):-
    noTowersOutsideVillage,
    eachVillageAtMostOneTower,                 % indica que cada ciutat pot tenir com a molt una torre
    realationTowerVillage,
    maxNumberOfTowers(MaxNumTowers),           % indica el nombre maxim de torres que hi poden haver en el mapa
    eachSignificantVillageOnTower,
    everyVillageObserved,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

noTowersOutsideVillage:- 
        findall(towerPos(I,J), (position(I,J), not(posVillage(V,I,J)), village(V)), Lits), exactly(0,Lits), fail.     
                % el mateix que (position(I,J), not(posVillage(_V,I,J)))
noTowersOutsideVillage.

eachVillageAtMostOneTower:- 
    village(V), findall(towerPos(I,J), posVillage(V,I,J), Lits ), atMost(1,Lits), fail.
eachVillageAtMostOneTower.

realationTowerVillage:- village(V), findall(towerPos(I,J), posVillage(V,I,J), Lits ), expressOr(hasTower(V),Lits), fail.
realationTowerVillage.

maxNumberOfTowers(MaxNumTowers):- findall(hasTower(V), village(V), Lits), atMost(MaxNumTowers, Lits).

eachSignificantVillageOnTower:-
    significantVillage(V), findall(towerPos(I,J), posVillage(V,I,J), Lits), exactly(1,Lits), fail.
eachSignificantVillageOnTower.

everyVillageObserved:-
        village(V), findall(towerPos(I,J), (posWatchesVillage(V,I,J), village(V2), posVillage(V2,I,J)),Lits), atLeast(1,Lits), fail.
everyVillageObserved.

% everyVillageObserved:-
%         village(V), findall(towerPos(I,J), posWatchesVillage(V,I,J), Lits), atLeast(1,Lits), fail.
% everyVillageObserved.
...
costOfThisSolution(M,Cost):- findall(I-J, member(towerPos(I,J), M), Lits), length(Lits,Cost), !.
