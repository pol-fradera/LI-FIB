...
writeClauses(MaxConsecutiveHours):-
    maxOneTaskPerHour,
    noConsecutiveDifferentTasks,
    gangsterNotAvailable,
    gangstersNeededPerHourAndTask,
    realtionWorksDoes,
    maxConsecHours(MaxConsecutiveHours),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

maxOneTaskPerHour:- gangster(G), hour(H), findall(does(G,T,H), task(T), Lits), atMost(1,Lits), fail.
maxOneTaskPerHour.

noConsecutiveDifferentTasks:- gangster(G), hour(H1), H2 is H1+1, hour(H2), task(T1), task(T2), T2 \= T1, 
                                writeOneClause([-does(G,T1,H1),-does(G,T2,H2)]), fail.
noConsecutiveDifferentTasks.

gangsterNotAvailable:- gangster(G), findall(does(G,T,H), (blocked(G,H), task(T)), Lits), exactly(0,Lits), fail.
gangsterNotAvailable.

gangstersNeededPerHourAndTask:- task(T), hour(H), findall(does(G,T,H), available(G,H), Lits), needed(T,H,N), exactly(N,Lits), fail.
gangstersNeededPerHourAndTask.

% realtionWorksDoes:- gangster(G), hour(H), writeOneClause([-works(G,H), does(G,_,H)])
...
maxConsecHours(MaxConsecutiveHours):- gangster(G), hour(H1), H2 is H1 + MaxConsecutiveHours, hour(H2),
                                      findall(-works(G,H), between(H1, H2, H), Lits),
                                      writeOneClause(Lits), fail.
maxConsecHours(_).

% works(G,H) <-> does(G,kill,H) v does(G,count,H) v does(G,poli,H) 
...
costOfThisSolution(M,Cost):- between(0, 72, H), Cost is 72-H, gangster(G), costOfThisGangster(M,G,Cost), !.

costOfThisGangster(M,G,Cost):- Hmax is 72-Cost+1, between(1, Hmax, Hini), horesConsecutives(M,G,Hini,Cost), !.

horesConsecutives(_,_,_,0).
horesConsecutives(M,G,Hini,C):- member( does(G,_,Hini), M), Hini1 is Hini+1, C1 is C-1, horesConsecutives(M,G,Hini1,C1).
