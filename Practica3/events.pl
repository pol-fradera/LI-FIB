...
satVariable( ed(E,D) ):- event(E), day(D).           % Expressa el següent: "el event E se celebra el dia D"
satVariable( em(E,M) ):- event(E), moderator(M).     % Expressa el següent: "el event E te assignat el moderador M"
satVariable( md(M,D) ):- moderator(M), day(D).       % Expressa el següent: "el moderador M treballa el dia D"

%%%%%%%  2. Clause generation for the SAT solver: =============================================

writeClauses:-
    eachEventExactlyOneDay,                 % a) Cada event es realitza en exactament un dia
    eachEventExactlyOneModerator,           % b) Cada event te exactamanet un moderador 
    maxEventsDay,                           % c) Indica el nombre maxim d'events que es poden realitzar en un dia
    relacioModeratorDay,                    % d) Relaciona moderador amb dia si l'event esta relacionat amb el dia i amb el moderador
    maxDaysModerator,                       % e) Indica el nombre de dies diferents que com a molt pot treballar un moderador
    true,!.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachEventExactlyOneDay:- event(E), eventDays(E,LD), findall( ed(E,D), member(D,LD), Lits ), exactly(1, Lits), fail.
eachEventExactlyOneDay.

eachEventExactlyOneModerator:- event(E), eventModerators(E,LM), findall( em(E,M), member(M,LM), Lits ), exactly(1, Lits), fail.
eachEventExactlyOneModerator.

maxEventsDay:- day(D), maxEventsPerDay(M), findall( ed(E,D), event(E), Lits ), atMost(M, Lits), fail.
maxEventsDay.

relacioModeratorDay:- moderator(M), day(D), event(E), eventModerators(E,LM), member(M,LM),
                        eventDays(E,LD), member(D,LD), writeOneClause([-ed(E,D), -em(E,M), md(M,D)]), fail.
relacioModeratorDay.                                                % ed(E,D) i em(E,M) -> md(M,D)
                                                                    % -ed(E,D) o -em(E,M) o md(M,D)

maxDaysModerator:- moderator(M), maxDaysPerModerator(N), findall( md(M,D), day(D), Lits ), atMost(N, Lits), fail.
maxDaysModerator.                                                                    
