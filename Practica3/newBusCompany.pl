...
cities([mad,bcn,val,bil,zar,sev]).
...
writeClauses:-  
    ...
    cityOfArrivalNextCityOfDeparture,
    eachTrajectAtMostOnePerDay,
    eachTrajectMinimalDaysPerWeek,
    maxDistanceConsecutiveDays,
    true,!.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

exactlyOneTripPerBusAndDay:- day(D), bus(B), findall(bdcc(B,D,C1,C2), trip(C1-C2), Lits), exactly(1,Lits), fail.
exactlyOneTripPerBusAndDay.

cityOfArrivalNextCityOfDeparture:- consecutiveDays(D1,D2), bus(B), trip(C1-C2), trip(C3-C4), C3 \= C2, 
                                        writeOneClause([-bdcc(B,D1,C1,C2),-bdcc(B,D2,C3,C4)]), fail.
cityOfArrivalNextCityOfDeparture.

eachTrajectAtMostOnePerDay:- trip(C1-C2), day(D), findall(bdcc(B,D,C1,C2), bus(B), Lits), atMost(1,Lits), fail.
eachTrajectAtMostOnePerDay.

eachTrajectMinimalDaysPerWeek:- trip(C1-C2), frequency(C1-C2,F), findall(bdcc(B,D,C1,C2), (day(D), bus(B)), Lits), atLeast(F,Lits), fail.
eachTrajectMinimalDaysPerWeek.

maxDistanceConsecutiveDays:- consecutiveDays(D1,D2), bus(B), dist(C1-C2,Dist1), dist(C2-C3,Dist2), maxDistance(Dist),
                             Dist1+Dist2 > Dist, writeOneClause([-bdcc(B,D1,C1,C2),-bdcc(B,D2,C2,C3)]), fail.
maxDistanceConsecutiveDays.
