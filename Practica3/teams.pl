...
incompatibleWorkers(W1,W2):- workerScore(W1,S1), workerScore(W2,S2), maxScore(M), W1 < W2, S is S1+S2, S > M.  %% Complete this!  % es poda W1 < W2 per evitar simetries i repeticions
...
writeClauses:-
    %% Complete this!  
    eachWorkerExactlyOneTeam,   % a) cada empleat I ha d'anar a exactament un equip
    minimalSize,               % b) la mida minima d'un equip es N
    maximalSize,               % c) la mida maxima d'un equip es M
    incompatibilities,         % d) prohibeix incompatibilitat de treballadors
    true,!.
...
minimalSize:- team(T), minSize(M), findall( wt(W,T), worker(W), Lits ), atLeast(M, Lits), fail.
minimalSize.

maximalSize:- team(T), maxSize(M), findall( wt(W,T), worker(W), Lits ), atMost(M, Lits), fail.
maximalSize.
